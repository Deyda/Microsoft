<#
        ...
        PartitionShrinkFailed	Failed to Optimze partition as part of the disk processing
        ...
        .EXAMPLE
        FSLogix-ShrinkDisk.ps1 -Path \\server\share -Recurse -IgnoreLessThanGB 3
        This shrinks all disks in the specified share recursively, except for files under 3GB in size which it ignores.
        .EXAMPLE
        FSLogix-ShrinkDisk.ps1 -Path \\server\share -Recurse -DeleteOlderThanDays 90
        ...
        .EXAMPLE
        FSLogix-ShrinkDisk.ps1 -Path \\server\share -Recurse -LogFilePath C:\MyLogFile.csv
        This shrinks all disks in the specified share recursively and changes the default log file location to C:\MyLogFile.csv
        .EXAMPLE
        FSLogix-ShrinkDisk.ps1 -Path \\server\share -Recurse -PassThru
        ...
        .EXAMPLE
        FSLogix-ShrinkDisk.ps1 -Path \\server\share -Recurse -ThrottleLimit 20
    ...
    function Optimize-OneDisk {
        ...
        PROCESS {
            ...
            while ($retries -lt 30 -and $success -ne $true) {
                ...
                if ($diskPartResult -match 'successfully compacted|erfolgreich komprimiert') {
