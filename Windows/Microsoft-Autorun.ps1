...
& "Microsoft-Autorun.ps1" -list -registry -allusers -run evernote

Show all autorun entries in the registry for all users that contain the string "evernote"

.EXAMPLE

& "Microsoft-Autorun.ps1" -name "Edit Hosts File" -run "notepad.exe c:\windows\system32\drivers\etc\hosts" -description "Look at the hosts files"

Add a shortcut for the user called "Edit Hosts File" which will run the given command line

.EXAMPLE

& "Microsoft-Autorun.ps1" -name "Edit Hosts File" -remove
